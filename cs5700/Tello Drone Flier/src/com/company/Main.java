...
//            bytesToSent = request.getBytes(StandardCharsets.UTF_8);
            datagramPacket = new DatagramPacket(commandMessage.encode(), commandMessage.encode().length, droneAddress, dronePort);
            udpClient.send(datagramPacket);
            System.out.println("Sent " + commandMessage.messageContents + " bytes to " + droneAddress.toString() + ":" + dronePort);
